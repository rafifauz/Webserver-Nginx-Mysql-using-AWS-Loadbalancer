...
aws ec2 run-instances --image-id ami-06fb5332e8e3e577a --count 1 --instance-type t2.micro --key-name CilsyAWS --security-group-ids sg-0cecc5906da0ce444 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Nginx_DB}]' 'ResourceType=volume,Tags=[{Key=Name,Value=Mysql_DB}]' --user-data '#!/bin/bash
